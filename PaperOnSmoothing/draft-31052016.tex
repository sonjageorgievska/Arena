...
We use the least squares optimization method, which in our case takes the form of a chi-square data fit. Exact description of the method is beyond the scope of this paper and we refer the reader to \cite{bevington} for details. We note, however, that the statistical estimation of the position provides as also with standard deviation $\sigma$ of the estimation, which we will use in Sec. \ref{sec:density}.
\begin{comment}
...
denote by $\mu$ the average variance of ${X_1, X_2, ..., X_n}$ (note that $\mu < 1$ from the definition of $\{X_i\}_{i=1}^n$ ).  From \eqref{eq_Var} and \eqref{eq_covar} we have  
