...
Formally, let $R$ be an arbitrary region from the stadium. Let $\{mac_1, mac_2,...mac_n\}$ be all MAC devices detected at time $t$.  Let $X_i$ be a random variable defined by
...
For each bin location $X$, in the time interval $t$, we apply the kernel density estimation (KDE) method \cite{scott}\cite{silverman} given by
...
Formally,  (to Philip: now write formulas, please ) 
...
Thus, when estimating crowd density, we ignore the MAC devices that have a randomized address and at the end we multiply the crowd density by 1.0225, to account for the ignored MAC devices. We choose factor 1.225 to allow for a safety margin: in the peaks of the graph in Fig. \ref{fig:randomized} the values are 0.225, and we would rather overestimate than underestimate the crowd density. 
