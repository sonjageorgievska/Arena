...
The uncertainty values in the parameters that correspond to a $1\sigma$ error or an increase of $\chi^2+1$ are given by the elements in the covariance matrix
\begin{equation}
C=H^\top V^{-1}H
\end{equation}
where $V$ is the covariance matrix of the measurement values, and $H$ is a projection matrix consisting of the partial derivatives of the path loss model with respect to the fit parameters.
